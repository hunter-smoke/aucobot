# `app/(auth)/` — Email OTP login & register (đã chốt)
...
Sau auth thành công → redirect `/` (sẽ chuyển `/(main)` khi có shell chat).
...
```text
app/(auth)/
  login/page.tsx + _components/ClientLoginPage/
  register/page.tsx + _components/ClientRegisterPage/
...
| Cũ | Hành động |
|----|-----------|
| `app/_components/AuthDemo/` | Đã xóa |
| `app/verify-email/` | Redirect → `/login` |
